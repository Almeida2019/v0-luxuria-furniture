  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a] text-[#f5f5f5]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="font-serif text-4xl md:text-5xl font-bold text-[#f5f5f5] mb-4">Our Collections</h1>
          <p className="text-xl text-[#f5f5f5]/80 max-w-3xl">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/4">
            <div className="bg-[#2d2d2d] rounded-lg p-6 border border-[#d4af37]/20">
              <div className="flex items-center gap-2 mb-6">
                <Filter className="w-5 h-5 text-[#d4af37]" />
                <h2 className="text-lg font-semibold text-[#f5f5f5]">Filters</h2>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-[#f5f5f5]/60" />
                  <Input
                    type="text"
                    placeholder="Search products..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="pl-10 bg-[#1a1a1a] border-[#d4af37]/30 focus:border-[#d4af37] text-[#f5f5f5]"
                  />
                </div>
              </div>
              ...
              {/* Category Filter */}
              <div className="mb-6">
                <h3 className="text-sm font-medium text-[#f5f5f5] mb-3">Category</h3>
                <div className="space-y-2">
                  {categories.map((category) => (
                    <div key={category} className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor={category} className="text-sm text-[#f5f5f5]/80 cursor-pointer">
                        {category}
                      </label>
                    </div>
                  ))}
              ...
              {/* Material Filter */}
              <div className="mb-6">
                <h3 className="text-sm font-medium text-[#f5f5f5] mb-3">Material</h3>
                <div className="space-y-2">
                  {materials.map((material) => (
                    <div key={material} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        className="border-[#d4af37]/30 data-[state=checked]:bg-[#d4af37] data-[state=checked]:border-[#d4af37]"
                      />
                      <label htmlFor={material} className="text-sm text-[#f5f5f5]/80 cursor-pointer">
                        {material}
                      </label>
                    </div>
                  ))}
              ...
              {/* Price Range */}
              <div className="mb-6">
                <h3 className="text-sm font-medium text-[#f5f5f5] mb-3">Price Range</h3>
              ...
              <Button
                variant="outline"
                className="w-full border-[#d4af37] text-[#d4af37] hover:bg-[#d4af37] hover:text-[#1a1a1a] bg-transparent"
                onClick={() => {
          ...
          <div className="lg:w-3/4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <Select value={sortBy} onValueChange={setSortBy}>
                  <SelectTrigger className="w-48 bg-[#2d2d2d] border-[#d4af37]/30 focus:border-[#d4af37] text-[#f5f5f5]">
                ...
                {/* View Mode */}
                <div className="flex items-center border border-[#d4af37]/30 rounded-md">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setViewMode("grid")}
                    className={`${viewMode === "grid" ? "bg-[#d4af37] text-[#1a1a1a]" : "text-[#f5f5f5] hover:text-[#d4af37]"}`}
                  >
                  ...
                  <Button
                    ...
                    className={`${viewMode === "list" ? "bg-[#d4af37] text-[#1a1a1a]" : "text-[#f5f5f5] hover:text-[#d4af37]"}`}
                  >
            ...
            >
              {currentProducts.map((product) => (
                <Card
                  key={product.id}
                  className={`group overflow-hidden bg-[#2d2d2d] border border-[#d4af37]/20 hover:border-[#d4af37] transition-all duration-500 shadow-lg hover:shadow-2xl hover:shadow-[#d4af37]/10 transform hover:-translate-y-2 ${viewMode === "list" ? "flex flex-row" : ""}`}
                >
                  <div className={`relative overflow-hidden ${viewMode === "list" ? "w-1/3" : ""}`}>
                    ...
                    <div className="absolute top-4 left-4 flex flex-col gap-2">
                      {product.isNew && <Badge className="bg-[#d4af37] text-[#1a1a1a] shadow-lg">New</Badge>}
                  ...
                  <CardContent className={`p-6 ${viewMode === "list" ? "w-2/3 flex flex-col justify-between" : ""}`}>
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center space-x-1">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`h-4 w-4 ${i < product.rating ? "fill-[#d4af37] text-[#d4af37]" : "text-[#f5f5f5]/30"}`}
                            />
                          ))}
                          <span className="text-sm text-[#f5f5f5]/60 ml-1">({product.reviews})</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {product.originalPrice && (
                          <span className="text-lg text-[#f5f5f5]/60 line-through">
                    ...
                    {viewMode === "list" && (
                      <div className="mt-4 flex gap-2">
                        <Button
                          className="flex-1 bg-[#d4af37] text-[#1a1a1a] hover:bg-[#d4af37]/90"
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="border-[#d4af37] text-[#d4af37] hover:bg-[#d4af37] hover:text-[#1a1a1a] disabled:opacity-50"
                >
                ...
                <div className="flex items-center gap-1">
                  {[...Array(totalPages)].map((_, i) => (
                    <Button
                      ...
                      className={`w-10 h-10 ${currentPage === i + 1 ? "bg-[#d4af37] text-[#1a1a1a]" : "border-[#d4af37] text-[#d4af37] hover:bg-[#d4af37] hover:text-[#1a1a1a]"}`}
                    >
                      {i + 1}
                    </Button>
                  ))}
